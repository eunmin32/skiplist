...
private:
  ...
  //Connect the bottom node and top node
  void connectToTop(SNode* bottom, SNode* top);

  //Connect the left node and right node
  void connectToRight(SNode* left, SNode* right);
  ...
  //Recursive helper function of add function
  //return ture if the value is successfully added to list
  bool add(const int Data, int Level, bool coinFlip, SNode* BottomNode);

  //return the pointer to the most top node containing the certain value.
  //return nullptr when the node is not found. 
  SNode* getTopPointerTo(const int Data);
